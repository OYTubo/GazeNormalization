...
# import sys
# sys.path.append('D:/Deeplearning/EVE_SCPT-master/src')
import torch
...
class StRefine(object):
    ...
    def refine(self, PoG_px, PoG_history, return_full=False):
        ...
        # try:
        combmap, is_valid = self.creat_combmap(torch.from_numpy(PoG_px).detach().cpu().numpy(), torch.from_numpy(PoG_history).detach().cpu().numpy())
        # except:
        #     st()
